...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.99\textwidth, keepaspectratio]{content/05_design_and_dev_artefacts/DataLayer.drawio.pdf}
...
\subsubsection{Consolidated Architectural Summary}\label{subsubsec:completeArchitecture}
...
\begin{itemize}
    \item Diagram A depicts the process of an experiment in general. The process part "Experiment Step" is representative for all other Swim Lane process diagrams.
    \item The "User" lane of the diagrams represents the interaction of the user. The individual process steps in this lane are therefore represented by UI activities.
    \item The lane "Experiment Data" and "Participant Data" are representative for all activities which are associated with data of the respective parts. 
    \item The actual data resides in various data sources and is abstracted from the rest of the application by repositories. Although the two swim lanes are representative of the data layer, data source and repository are not mapped into the swim lanes.
    \item The individual process steps of the "Experiment Data" and "Participant Data" lanes are each represented by a single use case and thus form the domain layer.
\end{itemize}
